...
public class StatPanel : MonoBehaviour 
{
	...
	public void Display (GameObject obj)
	{
		...
		if (stats)
		{
			hpLabel.text = string.Format( "체력 {0} / {1}", stats[StatTypes.HP], stats[StatTypes.MHP] );
			mpLabel.text = string.Format( "마나 {0} / {1}", stats[StatTypes.MP], stats[StatTypes.MMP] );
			lvLabel.text = string.Format( "레벨. {0}", stats[StatTypes.LVL]);
		}
	}
}